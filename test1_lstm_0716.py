...
def one_conv_layer_net(sec_d,thr_d,for_d,first_filter_out,sec_filter_out,full_cn_out,filter1_size1,filter1_size2,filter2_size1,filter2_size2,iter_num,batch_size,train_pos,train_neg,max_pool1,max_pool2,vkmer,vkmer2,training_speed,seq_num,shape_num,file_list_all,file_list_random):
	...
	#matrix_K_L_M_train=np.concatenate((matrix_K_L_M[:train_pos],matrix_K_L_M_random[:train_neg]))
	#ys_train=np.repeat([[np.float32(1),np.float32(0)],[np.float32(0),np.float32(1)]], [matrix_K_L_M[:train_pos].shape[0],matrix_K_L_M_random[:train_neg].shape[0]], axis=0)
	...
	lstm_cell = tf.contrib.rnn.GRUCell(1)
	...
	### stop iteration if 20 successive 90% accuracy OR
	### if 5 successive 95% accuracy
	breaker1=np.repeat(0.5,20)
	breaker2=np.repeat(0.5,5)
	j1=0
	j2=0
	for i in range(iter_num):
		### mini batch postive part
		index_array_p=np.arange(matrix_K_L_M[:train_pos].shape[0])
		np.random.shuffle(index_array_p)
		batch_xs1_p=matrix_K_L_M[:train_pos][index_array_p[:int(batch_size/2)]]
		### mini batch negative part
		index_array_n=np.arange(matrix_K_L_M_random[:train_neg].shape[0])
		np.random.shuffle(index_array_n)
		batch_xs1_n=matrix_K_L_M_random[:train_neg][index_array_n[:int(batch_size/2)]]

		batch_xs1=np.concatenate((batch_xs1_p,batch_xs1_n))
		#batch_ys1=np.repeat([[np.float32(1),np.float32(0)],[np.float32(0),np.float32(1)]],[int(batch_size/2),int(batch_size/2)], axis=0)
		batch_ys1=np.repeat([[np.float32(1),np.float32(0)],[np.float32(0),np.float32(1)]],[int(batch_size/2),int(batch_size/2)], axis=0)

		sess.run(train_step, feed_dict={x: batch_xs1, y_: batch_ys1, keep_prob1: 0.5, keep_prob2: 0.5})
		if (i+1)%1000 == 0:
			#print(np.array(sess.run(outputs, feed_dict={x: batch_xs1, y_: batch_ys1, keep_prob1: 0.5, keep_prob2: 0.5})).shape)
			#print(sess.run(states, feed_dict={x: batch_xs1, y_: batch_ys1, keep_prob1: 0.5, keep_prob2: 0.5}))
			train_accuracy = accuracy.eval(feed_dict={x:batch_xs1, y_: batch_ys1, keep_prob1: 1.0, keep_prob2: 1.0})
			print("step %d, training accuracy %g"%(i, train_accuracy))
			train_accuracy = accuracy.eval(feed_dict={x:matrix_K_L_M_test, y_: ys_test, keep_prob1: 1.0, keep_prob2: 1.0})
			print("step %d, testing accuracy %g"%(i, train_accuracy))
			saver = tf.train.Saver()
			save_path = saver.save(sess, "trained_rnn.ckpt")
		...
		if (i+1)%100 == 0:
			...
			#train_accuracy = accuracy.eval(feed_dict={x:matrix_K_L_M_test, y_: ys_test, keep_prob1: 1.0, keep_prob2: 1.0})
			#print("step %d, testing accuracy %g"%(i, train_accuracy))
			breaker1[j1]=train_accuracy
			breaker2[j2]=train_accuracy
			#print(breaker)
			training_accuracy.write(str(i)+':\t'+str(train_accuracy)+'\n')
			if ((min(breaker1)>=0.99) or (min(breaker2)>=1)):
				break
			j1+=1
			j2+=1
			if j1==20:
			...
			if j2==5:
				j2=0
			#print(sess.run(h_conv1,feed_dict={x: matrix_K_L_M_visual_p, y_: ys_visual_p, keep_prob1: 1.0, keep_prob2: 1.0})[0,:,0,0])
		train_step.run(feed_dict={x: batch_xs1, y_: batch_ys1, keep_prob1: 0.5, keep_prob2: 0.5})

	hidden_state=np.array(sess.run(outputs, feed_dict={x: matrix_K_L_M_test, y_: ys_test, keep_prob1: 1.0, keep_prob2: 1.0}))
	np.savetxt('hidden_test.txt', hidden_state)

	W_atten_array=np.array(sess.run(W_atten, feed_dict={x: matrix_K_L_M_test, y_: ys_test, keep_prob1: 1.0, keep_prob2: 1.0}))
	np.savetxt('W_atten_array.txt', W_atten_array)

	#focus_out_array=np.array(sess.run(focus_out, feed_dict={x: matrix_K_L_M_test, y_: ys_test, keep_prob1: 1.0, keep_prob2: 1.0}))
	#np.savetxt('focus_out_array.txt', focus_out_array)

	test_accuracy=accuracy.eval(feed_dict={x: matrix_K_L_M_test, y_: ys_test, keep_prob1: 1.0, keep_prob2: 1.0})
	print("test accuracy %g"%test_accuracy)
	training_accuracy.write('test_accuracy:\t'+str(test_accuracy)+'\n')
...
# time python test1_lstm_0716_analysis.py -s 200 -t 1 -f 4 -i 8 -e 16 -u 16 -l 5 -o 1 -r 8 -v 1 -a 10000 -b 100 -p 4000 -n 4000 -x 1 -y 2 -k 2 -m 6 -d 0.0001 -j 4 -q 0 -w pos_list.txt -z neg_list.txt